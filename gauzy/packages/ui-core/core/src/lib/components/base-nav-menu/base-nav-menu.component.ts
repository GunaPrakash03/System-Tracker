...
export class BaseNavMenuComponent extends TranslationBaseComponent implements OnInit, AfterViewInit, OnDestroy {
		...
		return [
			{
				...
				data: {
					...
					// Admins and managers only — an employee's home is "My work".
					// The route carries the same permission, so this hides a link
					// that would otherwise bounce them straight back.
			...
			{
				// "My work" — the self-service page. Sits directly under Dashboards
				// because for an employee it *is* their dashboard: the three reads
				// they are allowed live in it as tabs, and it is where they land at
				// login. Admins and managers open the same page for whichever
				// employee the header selector names.
				id: 'my-work',
				title: 'My work',
				icon: 'fas fa-user-clock',
				link: '/pages/employees/my-work',
				pathMatch: 'prefix',
				data: {
					translationKey: 'MENU.MY_WORK',
					permissionKeys: [PermissionsEnum.TIME_TRACKER],
					featureKey: FeatureEnum.FEATURE_TIME_TRACKING
				}
			},
			{
				id: 'focus',
		...
		return [
			{
				...
				data: {
					...
					// Company structure — departments, teams, equipment. Nothing an
					// employee acts on, and Departments is reachable on its feature
					// flag alone, so the group needs its own gate.
					permissionKeys: [PermissionsEnum.ORG_EMPLOYEES_VIEW]
				},
				items: [
					{
		...
		return [
			{
				...
				items: [
					...
					{
						// System-Tracker settings. Gated on CHANGE_ROLES_PERMISSIONS
						// because Gauzy has no "is super admin" permission to test
						// and, per config/roles-manager-hr.sql, that permission is
						// held by SUPER_ADMIN alone. This only hides the entry —
						// the tracker's admin service verifies the caller's role
						// itself, so an admin who reached the page anyway is
						// refused by the server rather than by a hidden link.
						id: 'settings-tracker',
						title: 'Tracker Settings',
						link: '/pages/settings/tracker',
						icon: 'fas fa-desktop',
						data: {
							translationKey: 'MENU.TRACKER_SETTINGS',
							permissionKeys: [PermissionsEnum.CHANGE_ROLES_PERMISSIONS]
						}
					},
					{
	...
	public mapMenuSection(item: NavMenuSectionItem): NavMenuSectionItem {
		...
		if (item.items) {
			...
			// A parent with nothing left under it is hidden too.
			//
			// Most top-level groups — Accounting, Tasks, Employees, Organization,
			// Goals — carry no featureKey or permission of their own; only their
			// children do. So trimming features or restricting a role emptied the
			// group but still drew its header, giving an employee a sidebar full
			// of sections that open onto nothing. A group is only meaningful if
			// something inside it is reachable.
			//
			// Groups that are a link in their own right (a `link` with no items)
			// are unaffected: this only applies where `items` was declared.
			if (section.children.length && section.children.every((child) => child.hidden)) {
				section.hidden = true;
			}
		}
