...
export interface IEmployeeCreateInput extends IBasePerTenantAndOrganizationEntityMutationInput, ITaggable {
	...
	password?: string;
	/**
	 * Role to give the new user: EMPLOYEE, MANAGER or ADMIN. Defaults to
	 * EMPLOYEE when omitted, which is what every caller got before this existed.
	 * The server restricts which values it will honour — see the employee create
