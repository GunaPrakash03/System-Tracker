...
export class ActivityService extends TenantAwareCrudService<Activity> {
	...
	async getDailyActivities(request: IGetActivitiesInput): Promise<IDailyActivity[]> {
		switch (this.ormType) {
			case MultiORMEnum.MikroORM: {
				...
				const employeeIds: string[] = await this._managedEmployeeService.filterAccessibleEmployeeIds(
					request.employeeIds ?? [],
					[], // teamIds
					request.projectIds ?? [],
					false
	...
	private async filterQuery(request: IGetActivitiesInput): Promise<SelectQueryBuilder<Activity>> {
		const { organizationId, startDate, endDate } = request;
		const tenantId = RequestContext.currentTenantId();

		// Async because manager scoping needs a team-membership lookup. Admins get
		// the requested ids back untouched, so an empty request still means "all".
		const employeeIds: string[] = await this._managedEmployeeService.filterAccessibleEmployeeIds(
			request.employeeIds ?? [],
			[], // teamIds
			request.projectIds ?? [],
