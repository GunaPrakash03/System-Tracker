...
export class TimeSlotService extends TenantAwareCrudService<TimeSlot> {
	...
	async getTimeSlots(request: IGetTimeSlotInput) {
		...
		// Filter employeeIds by permission and manager access. This used to fall
		// straight back to the caller's own employeeId, which left a manager
		// unable to see the slots - and so the screenshots - of the people they
		// manage. No team is passed: this endpoint filters by employee and date.
