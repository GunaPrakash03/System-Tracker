...
export class EmployeeCreateHandler implements ICommandHandler<EmployeeCreateCommand> {
	...
	public async execute(command: EmployeeCreateCommand): Promise<IEmployee> {
		...
		if (isEmpty(input.userId)) {
			...
			// 1. Find the requested role for relative tenant, defaulting to EMPLOYEE
			const role = await this._roleService.findOneByWhereOptions({
				name: this.resolveRoleName(input.roleName),
	...
	/**
	 * Decides which role a newly created employee actually gets.
	 *
	 * The client sends a preference; this decides. Creating a user is a
	 * privilege-granting action, so the request is never trusted:
	 *
	 * - Anything outside EMPLOYEE / MANAGER / ADMIN falls back to EMPLOYEE.
	 *   SUPER_ADMIN in particular can never be granted this way — that would let
	 *   an admin mint an account outranking their own.
	 * - ADMIN may only be granted by a SUPER_ADMIN or ADMIN caller. Reaching this
	 *   code already requires ORG_EMPLOYEES_ADD, but that permission is grantable
	 *   to a custom role, so the check is made explicitly rather than assumed.
	 *
	 * @param requested - The role name asked for by the caller, if any
	 * @returns The role name to create the user with
	 */
	private resolveRoleName(requested?: RolesEnum): RolesEnum {
		if (requested === RolesEnum.MANAGER) {
			return RolesEnum.MANAGER;
		}

		if (requested === RolesEnum.ADMIN && RequestContext.hasRoles([RolesEnum.SUPER_ADMIN, RolesEnum.ADMIN])) {
