...
function getDashboardRoute(): Route[] {
	return [
		{
			// Admins and managers only. An employee's view is "My work"; the
			// organisation dashboard reports across people and projects, which is
			// not theirs to read. ORG_EMPLOYEES_VIEW is the separator used
			// throughout — employees do not hold it, managers and admins do.
			//
			// The guard doubles as the landing redirect: Gauzy sends everyone to
			// /pages/dashboard after login, so an employee arriving here is
			// forwarded to their own page instead of being shown an empty one.
			path: 'dashboard',
