...
export const env: Env = cleanEnv(
	process.env,
	{
		...
		//
		// This default is what actually ships. `packages/ui-config/.../environment*.ts`
		// is GENERATED by .scripts/configure.ts (it unlinks and rewrites both files)
		// and is gitignored, so editing the generated file brands only the machine
		// that ran the build — a fresh clone or a CI build reverts to whatever is
		// written here. That is why the login page kept showing the Ever Gauzy mark
		// even though the checked-out environment.prod.ts said otherwise.
		PLATFORM_LOGO: str({
			default: 'assets/images/logos/logo_young_globes.png'
		}),
